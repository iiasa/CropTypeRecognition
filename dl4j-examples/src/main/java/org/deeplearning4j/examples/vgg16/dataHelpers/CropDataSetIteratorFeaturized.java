...
package org.deeplearning4j.examples.vgg16.dataHelpers;

import org.deeplearning4j.datasets.iterator.AsyncDataSetIterator;
import org.deeplearning4j.examples.vgg16.FeaturizedPreSave;
 ...
 */
public class CropDataSetIteratorFeaturized {
    private static final Logger log = org.slf4j.LoggerFactory.getLogger(CropDataSetIteratorFeaturized.class);
    ...
    public static DataSetIterator trainIterator() throws UnsupportedKerasConfigurationException, IOException, InvalidKerasConfigurationException {
        runFeaturize();
        DataSetIterator existingTrainingData = new ExistingMiniBatchDataSetIterator(new File("trainFolder"),"crops-"+featureExtractorLayer+"-train-%d.bin");
    ...
    public static DataSetIterator testIterator() {
        DataSetIterator existingTestData = new ExistingMiniBatchDataSetIterator(new File("testFolder"),"crops-"+featureExtractorLayer+"-test-%d.bin");
    ...
    private static void runFeaturize() throws InvalidKerasConfigurationException, IOException, UnsupportedKerasConfigurationException {
        File trainDir = new File("trainFolder","crops-"+featureExtractorLayer+"-train-0.bin");
        if (!trainDir.isFile()) {
